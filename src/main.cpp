                ...
                for (unsigned int i = 0; i < tx.vout.size(); i++) {
                        ...
                        if(value != susAdrs.address(k)) { 
                            ...
                            if(value == address4.ToString().c_str() && i == outputIndex){
                                ...
                                if(checkAdr.isToBeBanned(nTime))    //if(banfromtime < (int64_t)nTime)  
                                    ...
                                    if(LockDebug) LogPrintf("Tx %s from  %s wasn't blocked since it has nTime %d earlier then specifyed %d timestamp (%s) or ban was dismissed.\n", GetHash().GetHex().c_str(), address4.ToString().c_str(),nTime, banfromtime, DateTimeStrFormat("%x %H:%M:%S", banfromtime));
                                } 
                            }
                        } 
                    } //  for(unsigned int k=0; k<susAdrs.sizeoflist(); k++)
                    ...
                    if(lockersAdr.getAdrValue(0) == address4.ToString().c_str() && i == outputIndex){ 
                        for (unsigned int k = 0; k < vout.size(); k++)
                        {
                            ...
                            CBuysellAddress address7(address6);
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    else if (nHeight > 210861 && nHeight <= 210864) {
        nSubsidy = 1 * COIN; // for secure mn start
    }
    else if (nHeight > 210864 && nHeight <= 210865) {
        nSubsidy = 5000000 * COIN; // for secure mn start
    }
    else if (nHeight > 210865 && nHeight <= 210965) {
        nSubsidy = 0.001 * COIN; // for secure mn start
    }
    else if (nHeight > 100) {
        nSubsidy = 0 * COIN; // 
    }
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    else if(nBestHeight >=   50000   &&  nBestHeight <   55940  ){ nSubsidy =   0.07 * COIN; }
    else if(nBestHeight >=   55940   &&  nBestHeight <  100000  ){ nSubsidy =   0.001 * COIN; }
    ...
    else if(nBestHeight >=  300000   &&  nBestHeight <  350000  ){ nSubsidy =   0.06 * COIN; }
    else if(nBestHeight >=  350000   &&  nBestHeight <  400000  ){ nSubsidy =   0.07 * COIN; }
    else if(nBestHeight >=  400000   &&  nBestHeight <  500000  ){ nSubsidy =   0.08 * COIN; }
    else if(nBestHeight >=  500000   &&  nBestHeight <  600000  ){ nSubsidy =   0.5 *  COIN; }
    else if(nBestHeight >=  600000   &&  nBestHeight <  700000  ){ nSubsidy =   0.1 *  COIN; }
    else if(nBestHeight >=  700000   &&  nBestHeight <  800000  ){ nSubsidy =   0.2 *  COIN; }
    else if(nBestHeight >=  800000   &&  nBestHeight < 1500000  ){ nSubsidy =   1.25 * COIN; }
    else if(nBestHeight >= 1500000)                              { nSubsidy =   0.01 * COIN; }
...
////////////////////////////////////////////////////////////////////////////////
    ...
    else if(height >  300000   &&  height <=  350000  ){ nSubsidy =   0.06 * COIN; }
    else if(height >  350000   &&  height <=  400000  ){ nSubsidy =   0.07 * COIN; }
    else if(height >  400000   &&  height <=  500000  ){ nSubsidy =   0.08 * COIN; }
    else if(height >  500000   &&  height <=  600000  ){ nSubsidy =   0.5 *  COIN; }
    else if(height >  600000   &&  height <=  700000  ){ nSubsidy =   0.1 *  COIN; }
    else if(height >  700000   &&  height <=  800000  ){ nSubsidy =   0.2 *  COIN; }
    else if(height >  800000   &&  height <= 1500000  ){ nSubsidy =   1.25 * COIN; }
    else if(height > 1500000)                          { nSubsidy =   0.01 * COIN; }
    else { nSubsidy = 0.1 * COIN; }
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    //  remove this first IF for any other then buysell coin
    if(nBestHeight > 200000){ // for BULL only for it was collateral change already before this code was added
    ...
    } // for BULL only for it was collateral change already before this code was added
                        ...
                        BOOST_FOREACH(const CTxIn& txin, tx.vin){
                            if (txin.prevout.IsNull()){
                                ...
                                for (unsigned int i = 0; i < vintx.vout.size(); i++) {
                                    ...
                                    CTxDestination address3;
                                    ExtractDestination(txout.scriptPubKey, address3);
                                    CBuysellAddress address5(address3);
            ...
            for (int w=2; w<block.vtx.size();w++)
            {
                ...
                for (unsigned int t = 0; t < bltx.vin.size(); t++){
                    ...
                    for (unsigned int i = 0; i < tx.vout.size(); i++) {
                        ...
                        // ----------------------------------------------------------
                        for(unsigned int k=0; k<scamAdrsSizeoflist; k++)
                        {
...
bool CBlock::getAllReceiversBySenderAddress(string addr) const   // reserved
{return true;}
        ...
        if(MasternodePayments && pindexBest->nHeight > STARTCHECKBLOCKS)
        {
            ...
            if(IsProofOfStake() && pindex != NULL){
                ...
                if(pindex->GetBlockHash() == hashPrevBlock){
                    ...
                    CBuysellAddress mnRewardPayee;
                    CBuysellAddress stRewardPayee;
                        ...
                        if(i!=0){
                            ...
                            if(i==1) {
                                ...
                                if(!vout1nVal){
                                    ...
                                    if(pindexBest->nHeight > STARTCHECKBLOCKS) return DoS(100, error("CheckBlock() : nValue in stake payment is not correct !!!"));
                                }
                                ...
                                if(vout2nVal){
                                    ...
                                    if(pindexBest->nHeight > STARTCHECKBLOCKS) return DoS(100, error("CheckBlock() : nValue in masternode payment is not correct !!!"));
                                } 
                            }
                        }
                    ...
                    if(!foundPaymentAndPayee) {
                        ...
                        if(pindexBest->nHeight > STARTCHECKBLOCKS) return DoS(100, error("CheckBlock() : Couldn't find masternode payment or winner payee"));
                    } 
                    else {
